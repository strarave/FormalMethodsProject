...
\begin{document}
	...
	\section{The Model}
		...
		\subsection{Bot}
			...
			\begin{itemize}
				...
				\item \textbf{\underline{Claimed}}: this state represents the bot after it claimed a task from the queue until it reaches the delivery point. The bot is forced to leave this state after \emph{K} seconds. The transition between \emph{Claimed} and \emph{Delivered} is regualated by the urgent channel \emph{podDelivered}. The channel is been set urgent because when the bot reaches the delivery point, no time should pass between bot arrival and message sending;
		...
		\subsection{Queue}
			...
			\begin{itemize}
				...
				\item A \emph{working} state, whose possible transitions are used to model the following features:
					...
					\end{enumerate}
			\end{itemize}
			\footnotetext{The choice of using a global variable instead of a meta one has been done because, in UPPAAL, it is not possible to include in an single transition the token exchanges on a channel and the information sent from the receiver to the sender. This is our particular case, so the alternative would have been to use an additional state just to exchange the meta variable. In order to keep the model as small as possible, we opted for the solution of the global variable. A third solution, namely switching sender/receiver role inside the two involved templates, is also possible, but its implementation would require the addition of few variables and a complication of the model.}
	...
	\section{System Configurations}
		We decided to show a schematic list of the configurations and the relative results and then discuss about the simulation outcomes in the next session.\\
