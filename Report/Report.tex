...
\begin{document}
	...
	\section{The Model}
		...
		At high level, the model is structured wiht a main template named \emph{Queue}. It receives tasks from the template \emph{Task} whenever a new one is ready and gives information about pod to claim to bots asking for a task.
		\subsection{Grid}
	...
	\section{Desired Properties}
		...
		\end{equation}
		Since we used SMC we do not aim to get a binary "the property holds / the property does not hold" but instead a probability range that provides information about the \emph{possibility} of such condition to not hold. Indeed, what we verified is the probability of the following formula:
		\begin{equation}
			\exists\, (\Diamond \text{ queue.discarded } > 0)
		\end{equation}
		where a high probability means the system loses some tasks, while a low probability means the system is able to successfully run.\\
	...
	\section{Conclusion}
		From the results showed, we were able to successfully find a configuration of the parameters where all the properties to be checked were satisfied. Moreover, the configuration that we were able to test successfully used 100000 seconds to carry out the run, and that represents more than 1400\footnote{Calculated considering that, on average, a task will be added every $\mu_t$ seconds} tasks carried out. %AIUTOOOOO @Simone %PREGOOOOO @Elia
